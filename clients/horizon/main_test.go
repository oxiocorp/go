...
func TestLoadAccountTransactions(t *testing.T) {
	...
	offers, err := client.LoadAccountTransactions("GCK6ALX65S5KTQRKX3OWG5DNCJ7XMI62N55TKRM6ZI2YNWDSO3PX3YSZ", Cursor("a"), Limit(50), OrderDesc)
	if assert.NoError(t, err) {
		assert.Equal(t, len(offers.Embedded.Records), 2)
	...
	offers, err = client.LoadAccountTransactions("GCK6ALX65S5KTQRKX3OWG5DNCJ7XMI62N55TKRM6ZI2YNWDSO3PX3YSZ", At("https://localhost/beepboop"))
	if assert.NoError(t, err) {
		assert.Equal(t, len(offers.Embedded.Records), 2)
		// TODO: Add proper asserts
		// assert.Equal(t, offers.Embedded.Records[0].ID, int64(161))
